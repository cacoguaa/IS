...
public class EaterAgentSCSV3 extends AgentSCSEater {
	...
	public int reactToAgent(){
		...
		if(e && waitSteps > 0){
			...
			if(waitSteps == MAXSTEPS){
				enemyMeetings [0]= 1;
				enemyMeetings [1]= (byte)posX;
				enemyMeetings [2]= (byte)posY;
				//system.out.println("guardo el punto");
			}
			waitSteps--;
			return -1;		
		}
	...
	public int toReturn(){
			...
			if (deviate > 0)destination = ndeviate.peek();
				...
				destination = oldDeviate;
				enemyMeetings [0]++;
				
				if (enemyMeetings [0] == 4) deadEnemy =true;
				if (change == true && deadEnemy == true) reset =true;
			}
